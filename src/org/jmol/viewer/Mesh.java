...
class Mesh {
	...
	void setTranslucent(boolean isTranslucent) {
		...
		if (vertexColixes != null)
			for (int i = vertexCount; --i >= 0;)
				vertexColixes[i] = Graphics3D.getColixTranslucent(vertexColixes[i], isTranslucent);
	}
	...
	void sumVertexNormals(Vector3f[] vectorSums) {
		...
		for (int i = polygonCount; --i >= 0;) {
			...
			try {
				if (pi != null) {
					Graphics3D.calcNormalizedNormal(vertices[pi[0]], vertices[pi[1]], vertices[pi[2]],
							vNormalizedNormal, vAB, vAC);
	...
	void checkForDuplicatePoints(float cutoff) {
		...
		for (int i = vertexCount; --i >= 0;)
			for (int j = i; --j >= 0;) {
				...
				if (dist2 < cutoff2) {
					Logger.debug("Mesh.checkForDuplicates " + vertices[i] + "<->" + vertices[j] + " : "
							+ Math.sqrt(dist2));
				}
			}
	}
	...
	final boolean isPolygonDisplayable(int index) {
		if (polygonIndexes == null || polygonIndexes[index] == null)
			return false; // XIE
		return (polygonIndexes[index].length > 0 && (modelIndex == index || modelFlags == null || modelFlags[index] != 0));
	}
