...
final public class JmolConstants {
	...
	public final static String version = "11.0.RC1";
	public final static String cvsDate = "$Date: 2007-03-28 01:54:32 $";
	...
	private final static String[] connectOperationStrings = { "delete", "modify", "create", "modifyOrCreate", "auto" };
	...
	private final static String[] pickingModeNames = { "off", "ident", "label", "center", "draw", "spin",
			"coord NOT IMPLEMENTED", "bond NOT IMPLEMENTED", "atom", "group", "chain", "molecule", "site", "element",
	...
	private final static String[] pickingStyleNames = { "toggle", "selectOrToggle", "extendedSelect", "measure",
			"measureoff" };
	...
	final static String[] bondOrderNames = { "single", "double", "triple", "aromatic", "hbond", "partial",
			"partialDouble" };

	final static short[] bondOrderValues = { BOND_COVALENT_SINGLE, BOND_COVALENT_DOUBLE, BOND_COVALENT_TRIPLE,
	...
	public final static int[] argbsHbondType = { 0xFFFF69B4, // unused - pink
	...
	/**
	 * The default elementSymbols. Presumably the only entry which may cause confusion is element 0, whose symbol we
	 * have defined as "Xx".
	 */
	public final static String[] elementSymbols = { "Xx", // 0
			"H", // 1
			"He", // 2
			"Li", // 3
			"Be", // 4
	...
	public final static String elementNames[] = { "unknown", // 0
			"hydrogen", // 1
			"helium", // 2
			"lithium", // 3
			"beryllium", // 4
	...
	private final static short[] altElementNumbers = { 0, 13, 16, 55, (2 << 7) + 1, // D = 2*128 + 1 <-- firstIsotope
			(3 << 7) + 1, // T = 3*128 + 1
			(13 << 7) + 6, // 13C
			(14 << 7) + 6, // 14C
			(15 << 7) + 7, // 15N
	...
	private final static String[] altElementSymbols = { "Xx", "Al", "S", "Cs", "D", "T", "13C", "14C", "15N", };

	private final static String[] altElementNames = { "dummy", "aluminium", "sulphur", "caesium", "deuterium",
			"tritium", "", "", "", };
	...
	/**
	 * Default table of van der Waals Radii. values are stored as MAR -- Milli Angstrom Radius Used for spacefill
	 * rendering of atoms. Values taken from OpenBabel.
	 * 
	 * @see <a href="http://openbabel.sourceforge.net">openbabel.sourceforge.net</a>
	 */
	public final static short[] vanderwaalsMars = { 1000, // 0 Xx big enough to see
			1200, // 1 H
			1400, // 2 He
			1820, // 3 Li
			1700, // 4 Be
	 ...
	 * @see <a href="http://openbabel.sourceforge.net">openbabel.sourceforge.net</a>
	 */
	private final static short[] covalentMars = { 0, // 0 Xx does not bond
			230, // 1 H
			930, // 2 He
			680, // 3 Li
			350, // 4 Be
	...
	/*******************************************************************************************************************
	 * ionic radii are looked up using an array of shorts (16 bits each) that contains the atomic number, the charge,
	 * and the radius in two consecutive values, encoded as follows:
	 * 
	 ...
	 * 
	 * This data is from Handbook of Chemistry and Physics. 48th Ed, 1967-8, p. F143 (scanned for Jmol by Phillip Barak,
	 * Jan 2004)
	 * 
	 * Reorganized from two separate arrays 9/2006 by Bob Hanson, who thought it was just too hard to look these up and,
	 * if necessary, add or modify. At the same time, the table was split into cations and anions for easier retrieval.
	 * 
	...
	private final static short[] cationLookupTable = { (3 << 4) + (1 + 4), 680, // "Li+1"
			(4 << 4) + (1 + 4), 440, // "Be+1"
			(4 << 4) + (2 + 4), 350, // "Be+2"
			(5 << 4) + (1 + 4), 350, // "B+1"
			(5 << 4) + (3 + 4), 230, // "B+3"
	...
	private final static short[] anionLookupTable = { (1 << 4) + (-1 + 4), 1540, // "H-1"
			(6 << 4) + (-4 + 4), 2600, // "C-4"
			(7 << 4) + (-3 + 4), 1710, // "N-3"
			(8 << 4) + (-2 + 4), 1320, // "O-2"
			(8 << 4) + (-1 + 4), 1760, // "O-1"
	...
	private final static byte[] paletteIDs = { PALETTE_NONE, PALETTE_CPK, PALETTE_PARTIAL_CHARGE,
			PALETTE_FORMAL_CHARGE, PALETTE_TEMP,

			PALETTE_FIXEDTEMP, PALETTE_SURFACE, PALETTE_STRUCTURE, PALETTE_AMINO,

			PALETTE_SHAPELY, PALETTE_CHAIN, PALETTE_GROUP, PALETTE_MONOMER, PALETTE_MOLECULE, PALETTE_ALTLOC,

			PALETTE_INSERTION, PALETTE_JMOL, PALETTE_RASMOL, PALETTE_TYPE };
	...
	static {
		...
		if ((elementSymbols.length != elementNames.length) || (elementSymbols.length != vanderwaalsMars.length)
				|| (elementSymbols.length != covalentMars.length) || (elementSymbols.length != argbsCpk.length)) {
	...
	/*******************************************************************************************************************
	 * In DRuMS, RasMol, and Chime, quoting from http://www.umass.edu/microbio/rasmol/rascolor.htm
	 * 
	 * The RasMol structure color scheme colors the molecule by protein secondary structure.
	 * 
	 * Structure Decimal RGB Hex RGB Alpha helices red-magenta [255,0,128] FF 00 80 * Beta strands yellow [255,200,0] FF
	 * C8 00 *
	 * 
	 * Turns pale blue [96,128,255] 60 80 FF Other white [255,255,255] FF FF FF
	 * 
	 * *Values given in the 1994 RasMol 2.5 Quick Reference Card ([240,0,128] and [255,255,0]) are not correct for
	 * RasMol 2.6-beta-2a. This correction was made above on Dec 5, 1998.
	 ******************************************************************************************************************/
	public final static int[] argbsStructure = { 0xFFFFFFFF, // PROTEIN_STRUCTURE_NONE
	...
	public final static int[] argbsAmino = { 0xFFBEA06E, // default tan
	...
	public final static int[] argbsShapely = { 0xFFFF00FF, // default
	 ...
	 * some pastel colors
	 * 
	 * C0D0FF - pastel blue B0FFB0 - pastel green B0FFFF - pastel cyan FFC0C8 - pink FFC0FF - pastel magenta FFFF80 -
	 * pastel yellow FFDEAD - navajowhite FFD070 - pastel gold
	 * 
	 * FF9898 - light coral B4E444 - light yellow-green C0C000 - light olive FF8060 - light tomato 00FF7F - springgreen
	 * 
	...
	public final static short FORMAL_CHARGE_COLIX_RED = (short) elementSymbols.length;
	public final static short FORMAL_CHARGE_COLIX_WHITE = (short) (FORMAL_CHARGE_COLIX_RED + 4);
	public final static short FORMAL_CHARGE_COLIX_BLUE = (short) (FORMAL_CHARGE_COLIX_WHITE + 7);
	...
	public final static short PARTIAL_CHARGE_COLIX_RED = (short) (FORMAL_CHARGE_COLIX_BLUE + 1);
	public final static short PARTIAL_CHARGE_COLIX_WHITE = (short) (PARTIAL_CHARGE_COLIX_RED + 15);
	public final static short PARTIAL_CHARGE_COLIX_BLUE = (short) (PARTIAL_CHARGE_COLIX_WHITE + 15);
	...
	public final static int[] argbsRoygbScale = { 0xFFFF0000, 0xFFFF2000, 0xFFFF4000, 0xFFFF6000, 0xFFFF8000,
			0xFFFFA000, 0xFFFFC000, 0xFFFFE000,

			0xFFFFF000, // yellow gets compressed, so give it an extra boost

			0xFFFFFF00, 0xFFF0F000, // yellow gets compressed, so give it a little boost
			0xFFE0FF00, 0xFFC0FF00, 0xFFA0FF00, 0xFF80FF00, 0xFF60FF00, 0xFF40FF00, 0xFF20FF00,

			0xFF00FF00, 0xFF00FF20, 0xFF00FF40, 0xFF00FF60, 0xFF00FF80, 0xFF00FFA0, 0xFF00FFC0, 0xFF00FFE0,

			0xFF00FFFF, 0xFF00E0FF, 0xFF00C0FF, 0xFF00A0FF, 0xFF0080FF, 0xFF0060FF, 0xFF0040FF, 0xFF0020FF,

			0xFF0000FF, };

	/*
	 * public final static int[] argbsBlueRedRainbow = { 0xFF0000FF, //0xFF0010FF, 0xFF0020FF, //0xFF0030FF, 0xFF0040FF,
	 * //0xFF0050FF, 0xFF0060FF, //0xFF0070FF, 0xFF0080FF, //0xFF0090FF, 0xFF00A0FF, //0xFF00B0FF, 0xFF00C0FF,
	 * //0xFF00D0FF, 0xFF00E0FF, //0xFF00F0FF,
	 * 
	 * 0xFF00FFFF, //0xFF00FFF0, 0xFF00FFE0, //0xFF00FFD0, 0xFF00FFC0, //0xFF00FFB0, 0xFF00FFA0, //0xFF00FF90,
	 * 0xFF00FF80, //0xFF00FF70, 0xFF00FF60, //0xFF00FF50, 0xFF00FF40, //0xFF00FF30, 0xFF00FF20, //0xFF00FF10,
	 * 
	 * 0xFF00FF00, //0xFF10FF00, 0xFF20FF00, //0xFF30FF00, 0xFF40FF00, //0xFF50FF00, 0xFF60FF00, //0xFF70FF00,
	 * 0xFF80FF00, //0xFF90FF00, 0xFFA0FF00, //0xFFB0FF00, 0xFFC0FF00, //0xFFD0FF00, 0xFFE0FF00, //0xFFF0FF00,
	 * 
	 * 0xFFFFFF00, //0xFFFFF000, 0xFFFFE000, //0xFFFFD000, 0xFFFFC000, //0xFFFFB000, 0xFFFFA000, //0xFFFF9000,
	 * 0xFFFF8000, //0xFFFF7000, 0xFFFF6000, //0xFFFF5000, 0xFFFF4000, //0xFFFF3000, 0xFFFF2000, //0xFFFF1000,
	 * 
	 * 0xFFFF0000, };
	 */
	...
	public final static int[] argbsIsosurfacePositive = { 0xFF5020A0, 0xFF7040C0, 0xFF9060E0, 0xFFB080FF, };

	public final static int[] argbsIsosurfaceNegative = { 0xFFA02050, 0xFFC04070, 0xFFE06090, 0xFFFF80B0, };
	...
	private final static String allCarbohydrates = ",[AFL],[AGC],[AHR],[ARA],[ARB],[BDF],[BDR],[BGC],[BMA]"
			+ ",[FCA],[FCB],[FRU],[FUC],[FUL],[GAL],[GLA],[GLB],[GLC]"
			+ ",[GUP],[LXC],[MAN],[RAA],[RAM],[RIB],[RIP],[XYP],[XYS]"
			+ ",[CBI],[CT3],[CTR],[CTT],[LAT],[MAB],[MAL],[MLR],[MTT]"
			+ ",[SUC],[TRE],[ASF],[GCU],[MTL],[NAG],[NAM],[RHA],[SOR]" + ",[XYL]";// from Eric Martz
	...
	/*******************************************************************************************************************
	 * PDB file format spec says that the 'residue name' must be right-justified. However, Eric Martz says that some
	 * files are not. Therefore, we will be 'flexible' in reading the group name ... we will trim() when read in the
	 * field. So a 'group3' can now be less than 3 characters long.
	 ******************************************************************************************************************/
	...
	// taken from PDB spec
			...
			"1MA", // 36
			"AMO", "5MC", "OMC", "1MG", // 40
			"2MG", "M2G", "7MG", "G7M", "OMG", // 45
			"YG", "QUO", "H2U", "5MU", "4SU", // 50
			"PSU",
	 ...
	 * rmh
	 * 
	 * G Glycine Gly P Proline Pro A Alanine Ala V Valine Val L Leucine Leu I Isoleucine Ile M Methionine Met C Cysteine
	 * Cys F Phenylalanine Phe Y Tyrosine Tyr W Tryptophan Trp H Histidine His K Lysine Lys R Arginine Arg Q Glutamine
	 * Gln N Asparagine Asn E Glutamic Acid Glu D Aspartic Acid Asp S Serine Ser T Threonine Thr
	 */
	"", // 0 this is the null group

			"A", // 1
			"R", "N", "D", "C", "Q", "E", "G", "H", "I", "L", "K", "M", "F", "P", // 15 Proline
			"S", "T", "W", "Y", "V", "A", // 21 ASP/ASN ambiguous
	...
	public static String[] predefinedSets = {
			...
			"@backbone (protein,nucleic) & _a>0 & _a<=63", "@sidechain (protein,nucleic) & !backbone",
			"@base nucleic & !backbone",

			"@turn _structure=1", "@sheet _structure=2", "@helix _structure=3",
	...
	public final static String[] shapeClassBases = { "Balls", "Sticks", "Hsticks", "Sssticks", "Labels", "Vectors",
			"Measures", "Dots", "Backbone", "Trace", "Cartoon", "Strands", "MeshRibbon", "Ribbons", "Rockets", "Stars",
			"Halos", "Axes", "Bbcage", "Uccage", "Frank", "Echo", "Hover", "Polyhedra", "Dipoles", "Pmesh",
			"MolecularOrbital", "Isosurface", "LcaoCartoon", "Draw"
	// ,"Prueba","Sasurface"
	};
	...
	public final static int[] shapeToks = { Token.atom, Token.bonds, Token.hbond, Token.ssbond, Token.label,
			Token.vector, Token.monitor, Token.dots, Token.backbone, Token.trace, Token.cartoon, Token.strands,
			Token.meshRibbon, Token.ribbon, Token.rocket, Token.star, Token.halo, Token.axes, Token.boundbox,
			Token.unitcell, Token.frank, Token.echo, Token.hover, Token.polyhedra, Token.dipole, Token.pmesh, Token.mo,
			Token.isosurface, Token.lcaocartoon, Token.draw
