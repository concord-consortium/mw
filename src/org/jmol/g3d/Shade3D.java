...
final class Shade3D {
	...
	static float magnitudeLight = (float) Math.sqrt(xLightsource * xLightsource + yLightsource * yLightsource
	...
	// set specular on|off
	private static boolean specularOn = true;
	// set specular 0-100
	private static float intensitySpecular = 0.22f;
	// set specpower -6
	private static int specularExponent = 3;
	// set specpower 0-100
	private static float intenseFraction = 0.4f;
	// set diffuse 0-100
	private static float intensityDiffuse = 0.84f;
	// set ambient 0-100
	private static float ambientFraction = 0.45f;
	...
	static int[] getShades(int rgb, boolean greyScale) {
		...
		for (int i = 1; i <= nSteps; ++i) {
			shades[shadeNormal + i] = rgb(red + (int) (redRange * i / nSteps + 0.5f), grn
					+ (int) (grnRange * i / nSteps + 0.5f), blu + (int) (bluRange * i / nSteps + 0.5f));
		}
		if (greyScale)
	...
	static byte calcIntensity(float x, float y, float z) {
		...
		return (byte) (calcFloatIntensityNormalized((float) (x / magnitude), (float) (y / magnitude),
				(float) (z / magnitude))
				* shadeLast + 0.5f);
	}
	...
	static byte calcIntensityNormalized(float x, float y, float z) {
		...
		return (byte) (calcFloatIntensityNormalized(x, y, z) * shadeLast + 0.5f);
	}
	...
	static int calcFp8Intensity(float x, float y, float z) {
		...
		double magnitude = Math.sqrt(x * x + y * y + z * z);
		return (int) (calcFloatIntensityNormalized((float) (x / magnitude), (float) (y / magnitude),
				(float) (z / magnitude))
				* shadeLast * (1 << 8));
	}
	...
	/*
	 * This is a linear congruential pseudorandom number generator, as defined by D. H. Lehmer and described by Donald
	 * E. Knuth in The Art of Computer Programming, Volume 2: Seminumerical Algorithms, section 3.2.1.
	 * 
	 * static long seed = 1; static int nextRandom8Bit() { seed = (seed * 0x5DEECE66DL + 0xBL) & ((1L << 48) - 1); //
	 * return (int)(seed >>> (48 - bits)); return (int)(seed >>> 40); }
	 */
	 ...
	 * </p>
	 * <p>
	 * RANDU is the classic example of a poor random number generator. But it is very cheap to calculate and is good
	...
	static void setLightsourceZ(float z) {
		zLightsource = z;
		magnitudeLight = (float) Math.sqrt(xLightsource * xLightsource + yLightsource * yLightsource + zLightsource
				* zLightsource);
		dump();
	}
	...
	static void setSpecularPower(int specularPower) {
		...
		else specularExponent = -specularPower;
		dump();
	}
