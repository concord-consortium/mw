...
class Resolver {
	...
	static Object resolve(String name, BufferedReader bufferedReader, JmolAdapter.Logger logger, int[] params)
			throws Exception {
		...
		String className = "org.myjmol.adapter.smarter." + atomSetCollectionReaderName + "Reader";
	...
	/*
	 * XIE static Object DOMResolve(Object DOMNode, JmolAdapter.Logger logger) throws Exception {
	 * AtomSetCollectionReader atomSetCollectionReader = new XmlReader();
	 * 
	 * atomSetCollectionReader.setLogger(logger); atomSetCollectionReader.initialize();
	 * 
	 * AtomSetCollection atomSetCollection = atomSetCollectionReader.readAtomSetCollectionFromDOM(DOMNode); return
	 * finalize(atomSetCollection, "DOM node"); }
	 */
	...
	static String determineAtomSetCollectionReader(BufferedReader bufferedReader, JmolAdapter.Logger logger)
			throws Exception {
	...
	final static String[] nwchemRecords = { " argument  1 = " };

	final static String[] cubeRecords = { "JVXL" };

	final static String[] mol2Records = { "@<TRIPOS>" };

	final static String[] webmoRecords = { "[HEADER]" };

	final static String[][] fileStartsWithRecords = { nwchemRecords, cubeRecords, mol2Records, webmoRecords };

	final static String[] fileStartsWithFormats = { "NWChem", "Cube", "Mol2", "WebMO" };

	// //////////////////////////////////////////////////////////////
	// these test lines that startWith one of these strings
	// //////////////////////////////////////////////////////////////

	final static String[] pdbRecords = { "HEADER", "OBSLTE", "TITLE ", "CAVEAT", "COMPND", "SOURCE", "KEYWDS",
			"EXPDTA", "AUTHOR", "REVDAT", "SPRSDE", "JRNL  ", "REMARK", "DBREF ", "SEQADV", "SEQRES", "MODRES",
			"HELIX ", "SHEET ", "TURN  ", "CRYST1", "ORIGX1", "ORIGX2", "ORIGX3", "SCALE1", "SCALE2", "SCALE3",
			"ATOM  ", "HETATM", "MODEL ", };

	final static String[] shelxRecords = { "TITL ", "ZERR ", "LATT ", "SYMM ", "CELL " };

	final static String[] cifRecords = { "data_", "_publ" };

	final static String[] ghemicalMMRecords = { "!Header mm1gp", "!Header gpr" };

	final static String[] jaguarRecords = { "  |  Jaguar version", };

	final static String[] hinRecords = { "mol " };

	final static String[] mdlRecords = { "$MDL " };

	final static String[] spartanSmolRecords = { "INPUT=" };

	final static String[] csfRecords = { "local_transform" };

	final static String[][] lineStartsWithRecords = { pdbRecords, shelxRecords, cifRecords, ghemicalMMRecords,
			jaguarRecords, hinRecords, mdlRecords, spartanSmolRecords, csfRecords };

	final static String[] lineStartsWithFormats = { "Pdb", "Shelx", "Cif", "GhemicalMM", "Jaguar", "Hin", "Mol",
			"SpartanSmol", "Csf" };
	...
	final static String[] xmlRecords = { "<?xml", "<atom", "<molecule", "<reaction", "<cml", "<bond", ".dtd\"",
			"<list>", "<entry", "<identifier", "http://www.xml-cml.org/schema/cml2/core" };

	final static String[] gaussianRecords = { "Entering Gaussian System", "Entering Link 1", "1998 Gaussian, Inc." };

	final static String[] mopacRecords = { "MOPAC 93 (c) Fujitsu", "MOPAC2002 (c) Fujitsu",
			"MOPAC FOR LINUX (PUBLIC DOMAIN VERSION)" };

	final static String[] qchemRecords = { "Welcome to Q-Chem", "A Quantum Leap Into The Future Of Chemistry" };

	final static String[] gamessRecords = { "GAMESS" };

	final static String[] spartanRecords = { "Spartan" };

	final static String[] spartanBinaryRecords = { "|PropertyArchive" };

	final static String[] adfRecords = { "Amsterdam Density Functional" };

	final static String[][] containsRecords = { xmlRecords, gaussianRecords, mopacRecords, qchemRecords, gamessRecords,
			spartanBinaryRecords, spartanRecords, mol2Records, adfRecords, };

	final static String[] containsFormats = { "Xml", "Gaussian", "Mopac", "Qchem", "Gamess", "SpartanSmol", "Spartan",
			"Mol2", "Adf" };
}
