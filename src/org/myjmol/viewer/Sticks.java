...
class Sticks extends Shape {
	...
	void setSize(int size, BitSet bsSelected) {
		...
		BondIterator iter = (isBonds ? frame.getBondIterator(bsSelected) : frame.getBondIterator(myMask, bsSelected));
		if (iter == null)
			return;
	...
	void setProperty(String propertyName, Object value, BitSet bsSelected) {
		...
		if ("color" == propertyName) {
			...
			if (pid == JmolConstants.PALETTE_TYPE) {
				...
				BondIterator iter = (isBonds ? frame.getBondIterator(bsSelected) : frame.getBondIterator(myMask,
						bsSelected));
				while (iter.hasNext()) {
	...
	String getShapeState() {
		...
		for (int i = frame.bondCount; --i >= 0;) {
			...
			if (reportAll || bsSizeSet != null && bsSizeSet.get(i))
				setStateInfo(temp, i, "wireframe " + (bond.mad == 1 ? "on" : "" + (bond.mad / 2000f)));
			if (reportAll || bsOrderSet != null && bsOrderSet.get(i))
				setStateInfo(temp, i, "bondOrder " + JmolConstants.getBondOrderNameFromOrder(bond.order));
