...
class PolyhedraRenderer extends ShapeRenderer {
	...
	void render() {
		...
		for (int i = polyhedra.polyhedronCount; --i >= 0;) {
			short colix = polyhedra.colixes == null ? 0 : polyhedra.colixes[polyhedrons[i].centralAtom.atomIndex];
	...
	void render1(Polyhedra.Polyhedron p, short colix) {
		...
		for (int i = 0, j = 0; j < planes.length;)
			drawFace(colix, p.normixes[i++], vertices[planes[j++]], vertices[planes[j++]], vertices[planes[j++]],
					isAll, isFrontOnly);

		for (int i = 0, j = 0; j < planes.length;)
			fillFace(colix, p.normixes[i++], vertices[planes[j++]], vertices[planes[j++]], vertices[planes[j++]]);
	}
	...
	void drawFace(short colix, short normix, Atom atomA, Atom atomB, Atom atomC, boolean isAll, boolean isFrontOnly) {
		if (isAll || isFrontOnly && g3d.isDirectedTowardsCamera(normix)) {
			g3d.drawCylinderTriangle(Graphics3D.getColixTranslucent(colix, false), atomA.screenX, atomA.screenY,
					atomA.screenZ, atomB.screenX, atomB.screenY, atomB.screenZ, atomC.screenX, atomC.screenY,
					atomC.screenZ, 3);
		}
	}

	void fillFace(short colix, short normix, Atom atomA, Atom atomB, Atom atomC) {
		g3d.fillTriangle(colix, normix, atomA.screenX, atomA.screenY, atomA.screenZ, atomB.screenX, atomB.screenY,
				atomB.screenZ, atomC.screenX, atomC.screenY, atomC.screenZ);
	}
}
