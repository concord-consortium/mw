...
class Triangle3D {
	...
	private final static byte MAX_FIX = 10; // XIE
	...
	/*
	 * =============================================================== 2004 05 12 - mth I have been working hard to get
	 * the triangles to render correctly when lines are drawn only once. the rules were : a pixel gets drawn when 1. it
	 * is to the left of a line 2. it is under a horizontal line
	 * 
	 * this generally worked OK, but failed on small skinny triangles careful reading of Michael Abrash's book Graphics
	 * Programming Black Book Chapter 38, The Polygon Primeval, page 714 it says: Narrow wedges and one-pixel-wide
	 * polygons will show up spottily I do not understand why this is the case so, the triangle drawing now paints
	 * overlapping edges by one pixel
	 * 
	 * ==============================================================
	 */
	...
	void fillTriangle(int xScreenA, int yScreenA, int zScreenA, int xScreenB, int yScreenB, int zScreenB, int xScreenC,
			int yScreenC, int zScreenC, boolean useGouraud) {
		ax[0] = xScreenA;
		ax[1] = xScreenB;
		ax[2] = xScreenC;
		ay[0] = yScreenA;
	...
	void fillTriangle(boolean useGouraud) {
		...
		if (nLines > g3d.height * MAX_FIX) // XIE
			return;
	...
	private void generateRaster(int dy, int iN, int iS, int[] axRaster, int[] azRaster, int iRaster, Rgb16[] gouraud) {
		...
		if (gouraud != null) {
			...
			if (VERIFY) {
				...
				/*
				 * if (rgb16Base.getArgb() != rgb16sGouraud[iS].getArgb()) throw new NullPointerException();
				 */
			}
		}
	}
	...
	private void fillRaster(int y, int numLines, boolean useGouraud, boolean isClipped) {
		...
		if (isClipped) {
			for (; --numLines >= 0; ++y, ++i) {
				...
				if (pixelCount > 0)
					g3d.plotPixelsClipped(pixelCount, xW, y, azW[i], azE[i], useGouraud ? rgb16sW[i] : null,
			...
			for (; --numLines >= 0; ++y, ++i) {
				...
				if (pixelCount > 0)
					g3d.plotPixelsUnclipped(pixelCount, xW, y, azW[i], azE[i], useGouraud ? rgb16sW[i] : null,
							useGouraud ? rgb16sE[i] : null);
			}
		}
	}
