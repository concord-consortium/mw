...
/**
 * Ideally, all editing actions must be passed to this for final approval, as an editing action will most likely trigger
 * the reset of the movie recorder.
 * 
 * @author Charles Xie
 */
...
public abstract class ModelAction extends AbstractAction {
	...
	protected ModelAction(MDModel m) {
		setModel(m);
	}

	public ModelAction(MDModel m, Executable ex) {
		this(m);
		setExecutable(ex);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (myModel.hasEmbeddedMovie()) {
			if (myModel.resetTape() != JOptionPane.NO_OPTION) {
				...
				((MDView) myModel.getView()).clearEditor(false);
			}
		}
		else {
			executable.execute();
