...
public class MesoModelProperties extends ModelProperties {
	...
	private void setModel2(MesoModel m) {
		...
		label = new JLabel(model.getBoundary().getType() == RectangularBoundary.DBC_ID ? "(0, 0, " + 0.1f * dim.width
				+ ", " + 0.1f * dim.height + ")" : "(" + 0.1f * (float) model.getBoundary().x + ", " + 0.1f
				* (float) model.getBoundary().y + ", " + 0.1f * (float) model.getBoundary().width + ", " + 0.1f
				* (float) model.getBoundary().height + ")");
		label.setBorder(BUTTON_BORDER);
		p.add(label);
		p.add(new JLabel(" \u00c5"));
