...
public final class ChainMolecule extends Molecule {
	...
	public ChainMolecule(int typeOfAtom, int numberOfLinks, int growthMode, double length, double bondStrength, double angle, double bendStrength) {
		this.typeOfAtom = typeOfAtom;
		this.growthMode = growthMode;
		this.numberOfLinks = numberOfLinks;
		bondLength = length;
	...
	public void init(MolecularModel model) {
		...
		for (int k = 0; k < numberOfLinks; k++) {
			...
			model.atom[noa].setRx(k == 0 ? bondLength * Math.cos(angle) : model.atom[noa - 1].getRx() + bondLength * Math.cos(angle));
			model.atom[noa].setRy(k == 0 ? bondLength * Math.sin(angle) : model.atom[noa - 1].getRy() + bondLength * Math.sin(angle));
			addAtom(model.atom[noa]);
			noa++;

			if (k > 0) {
				bonds.add(new RadialBond.Builder(model.atom[noa - 1], model.atom[noa - 2]).bondLength(bondLength).bondStrength(bondStrength).build());
			}

			if (k > 1) {
				bends.add(new AngularBond(model.atom[noa - 1], model.atom[noa - 3], model.atom[noa - 2], Math.PI - bendAngle, bendStrength));
			}
