...
public class GayBerneParticle extends UnitedAtom implements Rotatable {
	...
	/*
	 * the factor that describes the distribution of mass in the elliptical area. The default value 1.0 corresponds to a uniform distribution.
	 */
	double mdFactor = 1.0;
	...
	public String toString() {
		return "Particle #" + getIndex() + " [ length=" + format.format(0.1 * length) + "nm, breadth=" + format.format(0.1 * breadth) + "nm ]";
	}
	...
	/*
	 * routine to correct the position predicted by the <tt>advance</tt> method. <b>Important</b>: <tt>fx, fy, tau</tt> were used in the force calculation routine to store the new acceleration data. <tt>ax, ay, alpha</tt> were used to hold the old acceleration data before calling this method. After calling this method, new acceleration data will be assigned to <tt>ax, ay, alpha</tt>, whereas the forces and torques to <tt>fx, fy, tau</tt>. <b>Be aware</b>: the acceleration and force properties of a particle are correct ONLY after this correction method has been called.
	 * 
	...
	/**
	 * Whether or not this GB particle intersects with the specified GB particle. This uses the paths generated by calling the <code> getPathIterator(AffineTransform at)</code> of the <code>Ellipse2D
	 *  </code> class to determine intersections.
	 * 
	...
	/**
	 * resize to a given dimension. This method checks that the breadth of the GB molecule cannot be longer than 60 and shorter than 10, whereas the length cannot be longer than 120 and shorter than 15.
	 * 
	 * @param x
	...
	public void render(Graphics2D g, AffineTransform at, AffineTransform savedAT) {
		...
		if (isVisible()) {
			...
			if (isBlinking()) {
				...
				g.drawOval((int) (rx - 0.6 * length), (int) (ry - 0.6 * breadth), (int) (1.2 * length), (int) (1.2 * breadth));
			}
	...
	private void convertToPolygon() {
		...
		while (!pi.isDone()) {
			...
			if (type == PathIterator.SEG_MOVETO || type == PathIterator.SEG_LINETO || type == PathIterator.SEG_CUBICTO || type == PathIterator.SEG_QUADTO) {
	...
	private void setDipLines() {
		...
		if (dipLine[3] == null) {
			dipLine[3] = new Line2D.Double(rx + half * length + 3 * Math.signum(half), ry, rx + half * length + 9 * Math.signum(half), ry);
		} else {
			dipLine[3].setLine(rx + half * length + 3 * Math.signum(half), ry, rx + half * length + 9 * Math.signum(half), ry);
		}
		if (dipLine[4] == null) {
			dipLine[4] = new Line2D.Double(rx + half * length + 6 * Math.signum(half), ry + 3, rx + half * length + 6 * Math.signum(half), ry - 3);
		} else {
			dipLine[4].setLine(rx + half * length + 6 * Math.signum(half), ry + 3, rx + half * length + 6 * Math.signum(half), ry - 3);
		}
		if (dipLine[5] == null) {
			dipLine[5] = new Line2D.Double(rx - half * length - 3 * Math.signum(half), ry, rx - half * length - 9 * Math.signum(half), ry);
		} else {
			dipLine[5].setLine(rx - half * length - 3 * Math.signum(half), ry, rx - half * length - 9 * Math.signum(half), ry);
		}
	}
