 ...
 * 
 */
class GaugeScripter extends ComponentScripter {

	private PageGauge gauge;

	GaugeScripter(PageGauge gauge) {
		super(true);
		this.gauge = gauge;
		setName("Gauge Script Runner #" + gauge.getIndex());
	}
	...
	protected void evalCommand(String ci) {
		if ("snapshot".equalsIgnoreCase(ci)) {
			EventQueue.invokeLater(new Runnable() {
				public void run() {
					SnapshotGallery.sharedInstance().takeSnapshot(gauge.getPage().getAddress(), gauge);
				}
			});
			return;
		}
