...
/**
 * This class controls the look and feel of axes. It is designed to be used in conjunction with the Graph2D class and
 * DataSet class for plotting 2D graphs.
 * 
 * To work with the other classes a system of registration is used. The axes have to be attached to the controlling
 * Graph2D class and the DataSet's have to be attached to both the Graph2D class and the Axis class.
 * 
 * This way the 3 main classes Graph2D, Axis and DataSet know of each others existence.
 * 
 * This does not mean the classes cannot be used independently, they can but in this mode nothing is automated, the user
 * must code everything manually
 * 
 * @version $Revision: 1.13 $, $Date: 2007-07-16 14:00:42 $.
...
public class Axis {
	...
	/**
	 * Default value <i>true</i>. Normally never changed. If set <i>false</I> the Axis draw method exits without
	 * drawing the axis.
	 * 
	 * @see org.concord.modeler.g2d.Axis#drawAxis(java.awt.Graphics)
	 */
	public boolean redraw = true;
	/**
	 * Rescale the axis so that labels fall at the end of the Axis. Default value <i>false</i>.
	 */
	...
	/**
	 * Minimum data value of the axis. This is the value used to scale data into the data window. This is the value to
	 * alter to force a rescaling of the data window.
	 */
	public double minimum;
	/**
	 * Maximum data value of the axis. This is the value used to scale data into the data window. This is the value to
	 * alter to force a rescaling of the data window.
	 */
	public double maximum;
	/**
	 * Before the Axis can be positioned correctly and drawn the data window needs to be calculated and passed to the
	 * Axis.
	 */
	 ...
	 */
	protected Vector<DataSet> dataset = new Vector<DataSet>();
	/**
	...
	/**
	 * If true the axis range must be manually set by setting the Axis.minimum and Axis.maximum variables. The default
	 * is false. The default action is for the axis range to be calculated everytime a dataset is attached.
	 */
	 ...
	 * 
	 * @param p
	 *            Set the axis position. Must be one of Axis.BOTTOM, Axis.TOP, Axis.LEFT, Axis.RIGHT, Axis.HORIZONTAL or
	...
	/** set the curve group this axis serves for */
	public void setCurveGroup(CurveGroup cg) {
		curveGroup = cg;
	}

	/** get the curve group this axis serves for */
	public CurveGroup getCurveGroup() {
		return curveGroup;
	}

	public void addCurveGroup(CurveGroup cg) {
	}
	 ...
	 * 
	 * @param p
	 *            Must be one of Axis.BOTTOM, Axis.TOP, Axis.LEFT, Axis.RIGHT, Axis.HORIZONTAL or Axis.VERTICAL. If one
	 *            of the latter two are used then Axis.BOTTOM or Axis.LEFT is assumed.
	 */
	...
	/**
	 * Return the pixel equivalent of the passed data value. Using the position of the axis and the maximum and minimum
	 * values convert the data value into a pixel value
	 * 
	 * @param v
	 *            data value to convert
	 * @return equivalent pixel value
	 * @see org.concord.modeler.g2d.Axis#getDouble(int)
	 */
	...
	/**
	 * Return the data value equivalent of the passed pixel position. Using the position of the axis and the maximum and
	 * minimum values convert the pixel position into a data value
	 * 
	 * @param i
	 *            pixel value
	 * @return equivalent data value
	 * @see org.concord.modeler.g2d.Axis#getInteger(double)
	 */
	 ...
	 *            The minimum X pixel
	 * @param xmax
	 *            The maximum X pixel. These should be equal if the axis is vertical
	 * @param ymin
	 *            The minimum Y pixel
	 * @param ymax
	 *            The maximum Y pixel. These should be equal if the axis is horizontal
	...
	public void setTitleFont(Font f) {
		title.setFont(f);
		if (curveGroup != null) {
			curveGroup.getLabelOfX().setFont(f);
			curveGroup.getLabelOfY().setFont(f);
		}
	}
	...
	/**
	 * Is the range of the axis to be set automatically (based on the data) or manually by setting the values
	 * Axis.minimum and Axis.maximum?
	 * 
	 * @param b
	 *            boolean value.
	 */
	...
	protected void drawHAxis(Graphics g) {
		...
		if (!exponent.isNull()) {
			if (position == TOP) {
				y0 = amin.y - label.getLeading(g) - label.getDescent(g) - exponent.getLeading(g)
						- exponent.getDescent(g);
			}
			else {
				y0 = amax.y + label.getLeading(g) + label.getAscent(g) + exponent.getLeading(g) + exponent.getAscent(g);
			}

			// x0 = amax.x; //Charles: exponent not seen
			x0 = amax.x - 10;
		...
		if (!title.isNull()) {
			if (position == TOP) {
				y0 = amin.y - label.getLeading(g) - label.getDescent(g) - title.getLeading(g) - title.getDescent(g);
			}
			else {
				y0 = amax.y + label.getLeading(g) + label.getAscent(g) + title.getLeading(g) + title.getAscent(g);
			}
	...
	protected void calculateGridLabels() {
		...
		else label_exponent = ((int) Math.floor(SpecialFunction.log10(Math.abs(maximum)) / 3.0)) * 3;
		...
		for (i = 0; i < label_count; i++) {
			...
			label_string[i] = formatter.format(val);
