...
/**
 * 
 * This class is an extension of the TextLine Class that allows text to be rotated. To rotate the text the
 * Component.createImage method is used. This means that this class needs to know the component that will receive the
 * text. If the component is not known the text cannot be rotated and the class will fall back to the TextLine.draw
 * method.
 * <P>
 * 
 * The text is rotated by running the image through an ImageFilter. This filter can easily be modified to rotate text
 * through an arbitrary angle but unless the text is large the integer mapping will blur the text. Also positioning of
...
public class RTextLine extends TextLine {
	...
	/**
	 * Set the Component the text will be drawn into. This is required to rotate the text. if it is not set the text
	 * will not be rotated.
	 * 
	 * @param c
	...
	public synchronized void draw(Component comp, Graphics g, int x, int y) {
		...
		/*
		 * Calculate the offset of the rotated image so that it will be positioned correctly. Remeber the image is
		 * calculated on the Maximum Ascent and descent so that no character is truncated
		 */
		switch (angle) {
		...
		/*
		 * Draw the rotated image to the Component. Do not notify any image consumer especially the component, otherwise
		 * we will get a feedback loop starting up, as this method is normally called from a paint method.
		 */
...
/*
 * This is an extension to the ImageFilter class that will rotate an image a multiple of 90 degrees. This filter is
 * easily extended to allow arbitrary rotations
 */
class RotateTextFilter extends ImageFilter {
	...
	/*
	 * The minimum and maximum points on the image after rotation. More important if arbitrary rotation was allowed
	 */
	private int xmin = 0;
	private int xmax = 0;
	private int ymin = 0;
	...
	/*
	 * The arrays that will store the new image. Only one is used depending on the data type of the original image.
	 */
	private int ipixels[];
	 ...
	 * Instantiate the class
	 * 
	 * @param angle the angle to rotate the image. Only multiples of 90 degrees are allowed
	 */
	 ...
	 * @param off
	 *            The offset into the pixels array where the parsed rectangle starts
	 * @param scan
	...
	public void setPixels(int x, int y, int w, int h, ColorModel model, byte pixels[], int off, int scan) {
		...
		/*
		 * If the byte array is null create it. Also remember the color model so that we can pass it onto the image
		 * consumer
		 */
	...
	public void setPixels(int x, int y, int w, int h, ColorModel model, int pixels[], int off, int scan) {
		...
		/*
		 * If the integer array is null create it. Also remember the color model so that we can pass it onto the image
		 * consumer
		 */
		if (ipixels == null) {
			colorModel = model;
	...
	/**
	 * Called when the image is complete. When this is called by the image producer, we can then pass the rotated image
	 * onto the image consumer.
	 * 
	 * @param status
	...
	public void imageComplete(int status) {
		...
		// Send the rotated image to the image consumer. Not forgetting to tell it when the image is complete
		if (bpixels != null) {
			for (int j = 0; j < height; j++)
				consumer.setPixels(0, j, width, 1, colorModel, bpixels, j * width, width);
			consumer.imageComplete(status);
		}
		else if (ipixels != null) {
			for (int j = 0; j < height; j++)
				consumer.setPixels(0, j, width, 1, colorModel, ipixels, j * width, width);
			consumer.imageComplete(status);
		}
		else consumer.imageComplete(ImageConsumer.IMAGEABORTED);
