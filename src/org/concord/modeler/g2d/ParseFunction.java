...
public class ParseFunction extends ScanString {
	...
	/*
	 * Specifiy the integer values associated with keywords. Every integer in this list is associated with keyword used
	 * in the string
	 */
	 ...
	 * 
	 * @return solution of the function
	 * @see org.concord.modeler.g2d.ParseFunction#setX(double)
	 * @see org.concord.modeler.g2d.ParseFunction#setY(double)
	 * @see org.concord.modeler.g2d.ParseFunction#setZ(double)
	 */
	...
	/**
	 * return a boolean array with index 0 true if the independent variable x was found in the function, index 1 true if
	...
	/*
	 * parse the string and build the node link list. This builds up a simple Left-Right binary node tree. A GROUP token
	 * (start of parenthesis) forces a new group node that starts an independent branch of the tree with the contents of
	 * the group linked to the left node of the group node. Intrinsic functions behave like a GROUP token they start an
	 * independent branch of the tree with the contents of the group linked to the left node of the intrinsic function.
	 * Intrinsic functions that have to be passed 2 parameters have the second parameter linked to there right node.
	 * (This has to be modified for functions containing more than 2 parameters currently the code cannot deal with more
	 * than 2 parameters)
	 */
	private int parseString(Node node) {
		...
		case RANDOM:
			 ...
			 * recurse using this node as it has to be used by an operator
			 */
	 ...
	 * first. Precedence Grouping Unary minus/plus Power Multiplication/Division Addition/Subtraction Variables,
	 * Intrinsic Functions, Constants
	 */
