...
String insertionString = null;
    ...
    protected void mutate(DNA dna,int strandIndex,int nucleotideIndex){
        if(dna == null) return;
        if(!dna.checkStrandComplementarity()) return;
        ...
        if(nucleotideIndex < 0) nucleotideIndex = 0;
        if(nucleotideIndex > firstStrand.getLength()) nucleotideIndex = firstStrand.getLength();
        Strand strandToInsert = generateStrandForInsertion();
        if(strandToInsert == null || strandToInsert.getLength() < 1) return;
        ...
        if(getMutationDirection() >= 0 && nucleotideIndex > 0) nucleotideIndex--;
    ...
    Strand generateStrandForInsertion(){
        if(insertionString == null) return Strand.generateRandomStrand(getFragmentLength());
    ...
    protected void mutate(Strand strand, int nucleotideIndex){
        ...
        if(nucleotideIndex < 0) nucleotideIndex = 0;
        if(nucleotideIndex > strand.getLength()) nucleotideIndex = strand.getLength();
