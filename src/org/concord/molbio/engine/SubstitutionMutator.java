...
String substitutionString = null;
    ...
    Nucleotide substituteNucleotide(Strand strand,int index,int substitutionIndex){
        if(substitutionString == null || substitutionIndex < 0 || substitutionIndex >= substitutionString.length()) return strand.substituteNucleotideRandomly(index);
    ...
    protected void mutate(Strand strand, int nucleotideIndex){
        if(strand == null || getFragmentLength() == 0) return;
