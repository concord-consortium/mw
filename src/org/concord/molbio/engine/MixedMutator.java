...
public class MixedMutator extends Mutator {
	...
	public void setMutationParam(Object[] params) {
		if (params == null || params.length != NUMB_POSSIBLE_MUTATORS)
			return;
		for (int i = 0; i < params.length; i++) {
			if (!(params[i] instanceof Integer))
				return;
		}
		randomizer = new MutatorRandomizer(((Integer) params[0]).intValue(), ((Integer) params[1]).intValue(),
	...
	protected void mutate(DNA dna, int strandIndex, int nucleotideIndex) {
		...
		if (mutator != null) {
			...
			notifyMutationListeners(new MutationEvent(mutator, strandIndex, nucleotideIndex, oldNucleotide,
					newNucleotide));
		}
	}
